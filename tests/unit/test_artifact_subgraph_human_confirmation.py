...
def artifact_state_with_confirmation(tmp_path, monkeypatch):
    ...
    (hc_dir / "confirmation_record.json").write_text(json.dumps({
        "final_status": "approved",
        "remaining_unconfirmed_fields": [],
    }))
    (hc_dir / "confirmed_model_plan.json").write_text(json.dumps({
...
async def test_review_report_shows_human_confirmation(artifact_state_with_confirmation):
    ...
    review = json.loads(
        (Path(result["review_artifact_dir"]) / "review_report.json").read_text()
    )
