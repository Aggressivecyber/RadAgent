...
def test_rejects_new_g4vsensitivedetector():
    result = scan_generated_code(
        _patch_with_content('auto sd = new G4VSensitiveDetector("sd");\n'), "test"
    )
    assert result["status"] == "fail"
...
def test_rejects_new_g4vuserdetectorconstruction():
    result = scan_generated_code(
        _patch_with_content("auto dc = new G4VUserDetectorConstruction();\n"), "test"
    )
    assert result["status"] == "fail"
...
def test_rejects_new_g4vuserprimarygeneratoraction():
    result = scan_generated_code(
        _patch_with_content("auto pg = new G4VUserPrimaryGeneratorAction();\n"), "test"
    )
    assert result["status"] == "fail"
...
def test_rejects_new_g4vuseractioninitialization():
    result = scan_generated_code(
        _patch_with_content("auto ai = new G4VUserActionInitialization();\n"), "test"
    )
    assert result["status"] == "fail"
