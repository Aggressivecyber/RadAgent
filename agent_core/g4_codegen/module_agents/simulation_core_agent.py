...
3. DetectorConstruction 必须根据 G4ModelIR components 建模 world 和子体层级，保持尺寸、母子关系、
   材料、位置和旋转一致。
   如果调用 OutputManager::SetGeometryDescription，必须传入第四个 shape 参数并保留
   G4ModelIR geometry_type；cylinder/tubs/cons 等非 box 不能省略 shape 而落入默认 box。
...
7. HPGe/BGO/屏蔽/外壳这类“包裹”几何不得生成同级实心重叠体。BGO veto 若包围 HPGe，
   必须是 shell/boolean subtraction，或用清晰 mother-daughter 层级表达；不能把 BGO
   实心圆柱和 HPGe 实心圆柱都放在 world 下同一中心。
8. voxel scoring 必须根据目标体尺寸选择有限 bin 数，并显式限制每轴 bin 数；不要默认 10 um
   覆盖厘米级探测器，避免 vector 分配触发 std::length_error / max_size。
9. Hit::Draw 使用 G4Circle 时必须 include "G4Circle.hh"；任何 G4ThreeVector 字段必须 include
   "G4ThreeVector.hh"；G4RotationMatrix 在 Geant4 11 中是 using/typedef 别名，不得写
   `class G4RotationMatrix;` 前向声明，使用时必须 include "G4RotationMatrix.hh"；
   使用 std::vector 必须 #include <vector>。
10. SensitiveDetector 的构造函数签名必须在 include/SensitiveDetector.hh、src/SensitiveDetector.cc
    和 DetectorConstruction 调用点完全一致；不得生成 SetScoringManager 这类 header 中不存在的方法。
11. 不得在 Hit/SensitiveDetector/ScoringManager 中留下 placeholder event_id/track_id 或固定 0 数据路径；
    需要事件/轨迹 id 时必须设计可由 runtime_app 从 G4Event/G4Track/G4Step 传入的接口。
12. SensitiveDetector 继承自 G4VSensitiveDetector；在其成员函数内分配项目自定义 Hit 时必须写
    `new ::Hit()`，不要写 `new Hit()`，避免被基类 G4VSensitiveDetector::Hit 名称隐藏。
13. 必须用 write_file 写文件；写完当前文件组所有文件后回复 DONE，不得输出 Markdown fence。
"""
