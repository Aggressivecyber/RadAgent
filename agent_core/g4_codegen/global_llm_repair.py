...
17. 必须通过 G4-G No Magic Number gate：除 0、1、2、0.0、1.0、2.0、0.5、
    180、360 和带 CLHEP 单位的数值外，所有数字必须先定义为具名 const/constexpr/enum，
    再在代码中引用。重点检查数组维度、buffer size、setprecision、mkdir 文件权限、
    G4ThreeVector 坐标、循环上界和 copyNo 维度计算。
18. 推荐修复模式：std::array<T, kAxisCount>，char buf[kCommandBufferSize]，
    std::setprecision(kCsvPrecision)，mkdir(path, kOutputDirectoryMode)，
    G4ThreeVector(0.0, 0.0, 1.0)；不要写 G4ThreeVector(0., 0., 1.)。
19. 使用 G4ExceptionDescription 时只 include "G4Exception.hh"；不要 include
    "G4ExceptionDescription.hh"。G4ExceptionDescription 类型由 G4Exception.hh 提供。
    ...
    *,
    ...
    repair_context = {
        ...
        "global_repair_policy": {
            ...
            "geant4_exception_description_include_rule": (
                'Use G4ExceptionDescription through "G4Exception.hh"; do not include '
                '"G4ExceptionDescription.hh".'
            ),
            "g4threevector_literal_rule": (
                "Use 0.0/1.0/2.0 inside G4ThreeVector, not 0./1./2.; promote "
                "other numeric values to named constants with units where applicable."
            ),
        },
    }
