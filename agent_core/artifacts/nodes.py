...
async def collect_artifacts(state: ArtifactSubgraphState) -> dict[str, Any]:
    ...
    # Target: review_artifacts/g4_complex_model/latest/ unless a test or caller
    # supplies an isolated artifact directory.
    project_root = Path(__file__).resolve().parent.parent.parent
    artifact_dir = Path(
        state.get("review_artifact_dir")
        or os.getenv("RADAGENT_REVIEW_ARTIFACT_DIR", "")
        or project_root / "review_artifacts" / "g4_complex_model" / "latest"
    )
...
async def generate_artifact_manifest(state: ArtifactSubgraphState) -> dict[str, Any]:
    ...
    try:
        ...
        logger.debug("Unable to resolve source git commit: %s", exc)
...
- **Gate Subgraph** → Gate 0-11 + G4-A to G4-H validation
"""
