...
class ParticleSpec(BaseModel):
    ...
    energy_MeV: float = Field(gt=0, description="Kinetic energy in MeV")
    direction: list[float] = Field(min_length=3, max_length=3, description="Unit direction vector [x, y, z]")
...
class TargetSpec(BaseModel):
    ...
    size_um: list[float] = Field(min_length=3, max_length=3, description="Dimensions [x, y, z] in um")
    geometry_type: str = Field(default="box", description="Geometry primitive: box, sphere, cylinder")


class DeviceSpec(BaseModel):
    """Semiconductor device specification."""
    device_type: str = Field(description="Device type, e.g. NMOS, PMOS, FinFET")
    temperature_K: float = Field(default=300.0, gt=0, description="Operating temperature in Kelvin")
...
class CircuitSpec(BaseModel):
    ...
    supply_voltage_V: float = Field(default=1.8, gt=0, description="Supply voltage in volts")
    ...
    model_config = {
        ...
        "json_schema_extra": {
            "examples": [{
                ...
                "particle": {"type": "proton", "energy_MeV": 100.0, "direction": [0.0, 0.0, -1.0], "events": 200000},
                "target": {"material": "Silicon", "size_um": [100.0, 100.0, 50.0], "geometry_type": "box"},
                "device": {"device_type": "NMOS", "temperature_K": 300.0, "bias_condition": "reverse_bias"},
                "outputs": ["dose_map", "tid_profile", "leakage_current"],
                "physics_options": {"physics_list": "FTFP_BERT", "em_physics": "G4EmStandardPhysics_option4"},
                "metadata": {"project": "TID_characterization"},
            }],
        },
    }

    simulation_scope: list[SimulationScope] = Field(min_length=1, description="Simulation domains to execute")
    particle: ParticleSpec | None = Field(default=None, description="Particle source configuration")
    target: TargetSpec | None = Field(default=None, description="Irradiation target configuration")
    device: DeviceSpec | None = Field(default=None, description="Device configuration")
    circuit: CircuitSpec | None = Field(default=None, description="Circuit configuration")
    outputs: list[str] = Field(default_factory=list, description="Requested output types")
    physics_options: dict[str, str] | None = Field(default=None, description="Physics list and model options")
