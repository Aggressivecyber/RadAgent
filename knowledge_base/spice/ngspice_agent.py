#!/usr/bin/env python3
"""
ngspice ReAct Agent — 思维链推理 Agent
- Thought → Action → Observation 循环
- 工具：search_ngspice, keyword_search, get_document, list_sources
- 自我评估结果是否足够回答问题
- 最多 5 轮迭代
"""
...
from ngspice_rag_mcp import search_documents, keyword_search, get_document, list_sources
...
AGENT_SYSTEM_PROMPT = """你是 ngspice/SPICE 电路仿真领域的专家助手。

你可以使用以下工具来查找信息：

1. search_ngspice(query, top_k) — 语义搜索 ngspice 文档和电路示例
   用途：查找手册说明、教程、网表示例
...
def execute_tool(action_name: str, params: dict) -> str:
    """执行单个工具调用"""
    try:
        if action_name in ("search_ngspice", "search_tcad"):
