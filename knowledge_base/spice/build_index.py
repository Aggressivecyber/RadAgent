#!/usr/bin/env python3
"""
ngspice RAG 向量索引构建脚本
- 读取 JSONL 文档
- 分块：手册按 512 tokens 分块（重叠 50），电路文件整文件一块
- 调用 Ollama bge-m3 嵌入
- 存储到 SQLite (data/ngspice_index.db)
...
DB_PATH = DATA_DIR / "ngspice_index.db"
...
NGSPICE_EXAMPLES_DIR = "/usr/share/doc/ngspice/examples"
        ...
        for attempt in range(max_retries):
            try:
                with opener.open(req, timeout=180) as resp:
                    ...
                    break
            except (urllib.error.URLError, KeyError, json.JSONDecodeError, TimeoutError, TimeoutError) as e:
...
def _scan_source_files() -> dict[str, tuple[float, int]]:
    ...
    if raw_dir.exists():
        for f in raw_dir.rglob("*"):
            if f.is_file() and f.suffix in ('.html', '.htm', '.texi'):
                sources[str(f)] = (f.stat().st_mtime, f.stat().st_size)
    examples_dir = Path(NGSPICE_EXAMPLES_DIR)
    if examples_dir.exists():
        for f in examples_dir.rglob("*"):
            if f.is_file() and f.suffix in ('.cir', '.sp', '.net', '.mod', '.lib', '.cmd', '.tcl', '.pro', '.script'):
                sources[str(f)] = (f.stat().st_mtime, f.stat().st_size)
    return sources
...
def main():
    parser = argparse.ArgumentParser(description="ngspice RAG 向量索引构建")
    parser.add_argument(
