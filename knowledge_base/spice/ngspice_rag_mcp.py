#!/usr/bin/env python3
"""
ngspice RAG MCP Server (stdio)
- 实现 MCP stdio JSON-RPC 协议
- 工具: search_ngspice, keyword_search, get_document, list_sources, ask_ngspice, generate_spice_code
"""
...
DB_PATH = Path(__file__).parent / "data" / "ngspice_index.db"
    ...
    if method == "initialize":
        ...
        send_response({
            "tools": [
                {
                    "name": "search_ngspice",
                    "description": "搜索 ngspice 相关文档。支持语义搜索手册、教程和电路示例。返回最相关的文档片段及其来源信息。",
                    "inputSchema": {
                        "type": "object",
                        "properties": {
                            "query": {
                                "type": "string",
                                "description": "搜索查询，支持自然语言，如 'MOSFET DC analysis' 或 '.tran transient simulation'"
                            },
                            "top_k": {
                                "type": "integer",
                ...
                {
                    "name": "keyword_search",
                    "description": "基于关键词精确搜索，补充语义搜索。适合搜索特定 SPICE 命令、模型名、参数等。",
                    "inputSchema": {
                        "type": "object",
                        "properties": {
                            "keyword": {
                                "type": "string",
                                "description": "搜索关键词，如 '.tran', 'BSIM', 'MOSFET'"
                            },
                            "top_k": {
                                "type": "integer",
                ...
                {
                    "name": "get_document",
                    "description": "根据文档 ID 获取完整内容。用于获取 search_ngspice 返回结果的完整内容。",
                    "inputSchema": {
                        "type": "object",
                        "properties": {
                            "doc_id": {
                ...
                {
                    "name": "list_sources",
                    "description": "列出所有 ngspice 数据源及其文档数量统计。",
                    "inputSchema": {
                        "type": "object",
                        "properties": {}
                    }
                ...
                {
                    "name": "ask_ngspice",
                    "description": "ngspice 专家问答（Agent 模式）。自动查询改写、多路检索、思维链推理。适合复杂技术问题。",
                    "inputSchema": {
                        "type": "object",
                        "properties": {
                            "query": {
                                "type": "string",
                                "description": "ngspice 问题，如 '如何仿真 CMOS 反相器的传输特性' 或 'how to do AC analysis'"
                            }
                        },
                        "required": ["query"]
                    }
                ...
                {
                    "name": "generate_spice_code",
                    "description": "根据需求生成 SPICE 网表代码。自动检索示例并结合需求生成完整网表，带中文注释。",
                    "inputSchema": {
                        "type": "object",
                        "properties": {
                            "requirements": {
                                "type": "string",
                                "description": "需求描述，如 'CMOS inverter transfer characteristic' 或 'RLC bandpass filter AC analysis'"
                            }
                        },
        ...
        try:
            ...
            elif tool_name == "ask_ngspice":
                query = arguments.get("query", "")
                if not query:
                    send_response({"error": {"code": -32602, "message": "缺少 query 参数"}}, req_id)
                    return
                try:
                    from ngspice_agent import run_agent
            ...
            elif tool_name == "generate_spice_code":
                requirements = arguments.get("requirements", "")
                if not requirements:
                    send_response({"error": {"code": -32602, "message": "缺少 requirements 参数"}}, req_id)
                    return
                try:
                    from spice_generator import generate_spice_code
                    code = generate_spice_code(requirements)
                    send_response({
