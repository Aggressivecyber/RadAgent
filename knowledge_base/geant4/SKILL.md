...
- 可选示例源码：设置 `RADAGENT_GEANT4_EXAMPLES_ROOT` 指向本机 Geant4 `examples/`
  目录；未设置时会尝试常见安装路径，找不到则跳过示例代码索引。

## 建索引

```bash
cd knowledge_base/geant4
...
```json
{
  "mcpServers": {
    "geant4-rag": {
      "command": "python3",
      "args": ["knowledge_base/geant4/geant4_rag_mcp.py"]
    }
  }
}
```
