...
def main() -> None:
    ...
    check(
        "  total_components = 9",
        summary.get("total_components") == 9,
        str(summary.get("total_components")),
    )
    ...
    check(
        "  materials_count = 5",
        summary.get("materials_count") == 5,
        str(summary.get("materials_count")),
    )
    check(
        "  scoring_count = 4", summary.get("scoring_count") == 4, str(summary.get("scoring_count"))
    )
    ...
    check(
        "  size > 100 bytes",
        rec_path.exists() and rec_path.stat().st_size > 100,
    ...
    if "schema_version" in record:
        check(
            ...
            record["schema_version"] == "confirmation_record_v1",
            record["schema_version"],
        )
    if "final_status" in record:
        check(
            "  final_status is approved/edited",
            record["final_status"] in {"approved", "edited"},
            record["final_status"],
        )
    if "remaining_unconfirmed_fields" in record:
        check(
            "  remaining_unconfirmed_fields empty",
            record["remaining_unconfirmed_fields"] == [],
            str(record["remaining_unconfirmed_fields"]),
        )
    if "confirmation_history" in record:
        check(
            "  confirmation_history >= 1 round",
            len(record["confirmation_history"]) >= 1,
            str(len(record["confirmation_history"])),
        )
    ...
    check(
        "  size > 100 bytes",
        plan_path.exists() and plan_path.stat().st_size > 100,
    ...
    if "remaining_unconfirmed_fields" in plan:
        check(
            "  remaining_unconfirmed_fields empty",
            plan["remaining_unconfirmed_fields"] == [],
            str(plan["remaining_unconfirmed_fields"]),
        )
    ...
    g4h = [
        g
        for g in gates
        if g.get("gate_id") in {19, "G4-H"} or "Human Confirmation" in g.get("name", "")
    ]
    ...
    if g4h:
        ...
        check(
            "  checked_items >= 4",
            len(g.get("checked_items", [])) >= 4,
            str(len(g.get("checked_items", []))),
        )
        check(
            "  passed_items non-empty",
            len(g.get("passed_items", [])) >= 4,
        ...
        check("  mentions confirmation_record", "confirmation_record" in str(g), str(g)[:200])
        check("  mentions confirmed_model_plan", "confirmed_model_plan" in str(g), str(g)[:200])
    ...
    check(
        ...
        review.get("validation_status") != "VERIFIED",
        review.get("validation_status"),
    )
    check("  is_stub = false", review.get("is_stub") is False, str(review.get("is_stub")))
    check(
        "  has_human_confirmation = true",
        review.get("has_human_confirmation") is True,
        str(review.get("has_human_confirmation")),
    )
    hc = review.get("human_confirmation", {})
    check("  hc.required = true", hc.get("required") is True, str(hc.get("required")))
    check(
        "  hc.status in approved/edited",
        hc.get("status") in {"approved", "edited"},
        str(hc.get("status")),
    )
    check("  hc.rounds >= 1", hc.get("rounds", 0) >= 1, str(hc.get("rounds")))
    check(
        "  hc.remaining_unconfirmed_fields empty",
        hc.get("remaining_unconfirmed_fields") == [],
        str(hc.get("remaining_unconfirmed_fields")),
    )
    ...
    check(
        "  summary materials_count = 5",
        ms.get("materials_count") == 5,
        str(ms.get("materials_count")),
    )
